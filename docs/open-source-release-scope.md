...
download the explicit watchlist scope, or one selected TWSE listed or TPEx
listed-on-the-board equity, for 1, 2, or 3 trailing years into the user
application-data directory. It also offers a separate explicit fundamentals
capture for the selected equity scope;
that action records only the latest admitted source periods and never runs in
the background. The app does not provide live quotes, broker access, order
placement, automatic execution, credentials, or background provider refresh.
Since 0.2.0 the desktop shell also offers an explicit,
