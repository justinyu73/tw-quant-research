...
桌面版可在人為選定個股後，按「更新台股資料」並選擇近 1、2 或 3 年，
從免費官方 TWSE 來源下載目前標的，將 raw 回應與標準化 K 線保存到本機
應用程式資料目錄。這不是即時更新、全市場下載或 TPEx 更新；瀏覽器預覽
不會下載資料。
...
對目前選取的 TWSE 上市個股抓取近 1～3 年，並保留 `as_of`、provenance
與來源 digest。TPEx、全市場與背景自動抓取仍未啟用。
